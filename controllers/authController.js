...
try {
    ...
    if(!user){
        res.status(404).json({error: "401 Credencial incorrecta"})
    }

    //comprobar la contraseña proporcionada con la almacena
    const isMatch = await bcrypt.compare(password, user.password)
    if(!isMatch){
        res.status(401).json({error: "Tu contraseña no coincide"})
    }

    //generar un toker con jwt
    const token = await jwt.sing({id: user.id}, SECRET_KEY, {expiresIn: "1h"});
